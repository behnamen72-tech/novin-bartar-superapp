...
def make_graph(session: Session):
    ...
    company_a = Organization(name="Company A", code="A", organization_type=OrganizationType.COMPANY, parent=holding)
    branch_a = Organization(name="Branch A", code="A-B", organization_type=OrganizationType.BRANCH, parent=company_a)
    company_b = Organization(name="Company B", code="B", organization_type=OrganizationType.COMPANY, parent=holding)
    person = Person(first_name="Access", last_name="User", email="access@example.com")
    user = User(person=person, email="access@example.com", username="accessuser", password_hash=hash_password("Strong-password-123!"))
...
def test_self_scope_allows_only_exact_organization() -> None:
    ...
    with Session(engine) as session:
        ...
        session.add(UserRoleAssignment(user=user, role=role, organization=company_a, scope_mode=OrganizationScopeMode.SELF))
        session.flush()
        assert has_permission(session, user_id=user.id, permission_code="people.read", organization_id=company_a.id) is True
        assert has_permission(session, user_id=user.id, permission_code="people.read", organization_id=branch_a.id) is False
...
def test_descendant_scope_allows_children_but_not_sibling_company() -> None:
    ...
    with Session(engine) as session:
        ...
        session.add(UserRoleAssignment(user=user, role=role, organization=company_a, scope_mode=OrganizationScopeMode.SELF_AND_DESCENDANTS))
        session.flush()
        assert has_permission(session, user_id=user.id, permission_code="people.read", organization_id=branch_a.id) is True
        assert has_permission(session, user_id=user.id, permission_code="people.read", organization_id=company_b.id) is False
...
def test_missing_permission_denies_even_with_role_assignment() -> None:
    ...
    with Session(engine) as session:
        ...
        session.add(UserRoleAssignment(user=user, role=role, organization=company_a, scope_mode=OrganizationScopeMode.SELF))
        session.flush()
        assert has_permission(session, user_id=user.id, permission_code="people.manage", organization_id=company_a.id) is False

        with pytest.raises(AuthorizationError):
            require_permission_for_organization(session, user_id=user.id, permission_code="people.manage", organization_id=company_a.id)
